...
public class GenericoDAO<T> {
    ...
    // Buscar lista com parâmetros (presente na 1ª versão e agora incluído)
    public List<T> buscarPorParametros(String comandoSql, RowMapper<T> rowMapper, Object... parametros) {
        List<T> entidades = new ArrayList<>();
        Connection con = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
    ...
    // Sobrecarga para compatibilidade: buscarTodos com parâmetros
public List<T> buscarTodos(String comandoSql, RowMapper<T> rowMapper, Object... parametros) {
    // delega à implementação que já faz o bind de parâmetros
    return buscarPorParametros(comandoSql, rowMapper, parametros);
}
